...
from litdata import StreamingDataLoader, CombinedStreamingDataset, StreamingDataset
...
class StreamingDatasetDataModule(LightningDataModule):
    ...
    def setup(self, stage):
        ...
        if stage not in _valid_stages:
            raise ValueError(f":stage must be one of {_valid_stages}, got {stage}")
        if stage in ("fit", "validate"):
            self.val_dataset: StreamingDataset = self.config.get_dataset("val") 
            if stage == "fit":
                self.train_dataset: StreamingDataset = self.config.get_dataset("train") 
        if stage == "test":
            self.test_dataset: StreamingDataset = self.config.get_dataset("test") 

    def train_dataloader(self) -> TRAIN_DATALOADERS:
        return StreamingDataLoader(self.train_dataset, shuffle = True, **self.config.get_dataloader_params())
    
    def val_dataloader(self) -> EVAL_DATALOADERS:
        return StreamingDataLoader(self.val_dataset, shuffle = False, **self.config.get_dataloader_params())

    def test_dataloader(self) -> EVAL_DATALOADERS:
        return StreamingDataLoader(self.test_dataset, shuffle = False, **self.config.get_dataloader_params())
