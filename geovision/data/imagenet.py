...
class ImagenetETL:
    ...
    def transform_to_hdf(cls, val_only: bool = False) -> None:
        ...
        hdf5 = fs.get_new_dir(cls.local, "hdf5") / "imagenet.h5"
        ...
        df.assign(image_path = lambda df: df["image_path"].str.removeprefix("ILSVRC/Data/CLS-LOC/")).to_hdf(hdf5, mode="w", key="df")
    ...
    def get_dataset_df_from_litdata(cls, config: Optional[DatasetConfig] = None, schema: Optional[pa.DataFrameSchema] = None) -> pd.DataFrame:
        try:
            ...
            return (
                DatasetConfig.get_df(config, schema, cls.get_dataset_df_from_imagefolder().reset_index(drop = False, names = "df_idx"))
                .assign(image_path = lambda df: df["image_path"].apply(lambda x: cls.local / "imagefolder" / Path(x)))
            )
    ...
    def get_dataset_df_from_imagefolder(cls) -> pd.DataFrame:
        ...
        imagefolder = fs.get_valid_dir_err(cls.local / "imagefolder")
        try:
            return pd.read_csv(fs.get_valid_file_err(imagefolder, "labels.csv"))
        except OSError:
            df = (
    ...
    def get_dataset_df_from_hdf5(cls) -> pd.DataFrame:
        ...
        return pd.read_hdf(fs.get_valid_file_err(cls.local, "hdf5", "imagenet.h5"), key="df", mode="r")  # type: ignore
...
class ImagenetteETL:
    ...
    def get_dataset_df_from_imagefolder(cls) -> pd.DataFrame:
        ...
        return pd.DataFrame({"image_path": list(fs.get_valid_dir_err(cls.local, "imagefolder", empty_ok=False).rglob("*.jpg"))}).pipe(cls._get_dataset_df)
    ...
    def get_dataset_df_from_hdf5(cls) -> pd.DataFrame:
        ...
        return pd.read_hdf(fs.get_valid_file_err(cls.local, "hdf5", "imagenette.h5"), key = "df", mode = 'r') # type: ignore
...
class ImagenetImagefolderClassification(Dataset):
    ...
    def __init__(self, split: Literal["train", "val", "trainval", "test", "all"] = "all", config: Optional[DatasetConfig] = None):
        ...
        self._df = self._config.verify_and_get_df(schema = self.df_schema, fallback_df = ImagenetETL.get_dataset_df_from_imagefolder())
...
class ImagenetHDF5Classification(Dataset):
    ...
    def __init__(self, split: Literal["train", "val", "trainval", "test", "all"] = "all", config: Optional[DatasetConfig] = None):
        ...
        self._config = config or ImagenetETL.default_config
        logger.info(f"init {self.name}[{self._split}]\nimage_pre = {self._config.image_pre}\ntarget_pre = {self._config.target_pre}\ntrain_aug = {self._config.train_aug}\neval_aug = {self._config.eval_aug}")
        self._df = self._config.verify_and_get_df(schema = self.df_schema, fallback_df = ImagenetETL.get_dataset_df_from_hdf5())
        self._split_df = self._config.verify_and_get_split_df(df = self._df, schema = self.split_df_schema, split = self._split, prefix_root = self._root)
...
class ImagenetteImagefolderClassification(Dataset):
    ...
    def __init__(self, split: Literal["train", "val", "trainval", "test", "all"] = "all", config: Optional[DatasetConfig] = None) -> None:
        ...
        logger.info(f"init {self.name}[{self._split}]\nimage_pre = {self._config.image_pre}\ntarget_pre = {self._config.target_pre}\ntrain_aug = {self._config.train_aug}\neval_aug = {self._config.eval_aug}")
        self._df = self._config.verify_and_get_df(schema = self.df_schema, fallback_df = ImagenetteETL.get_dataset_df_from_imagefolder())
        self._split_df = self._config.verify_and_get_split_df(df = self._df, schema = self.split_df_schema, split = self._split, prefix_root = self._root)
...
class ImagenetteHDF5Classification(Dataset):
    ...
    def __init__(self, split: Literal["train", "val", "trainval", "test", "all"] = "all", config: Optional[DatasetConfig] = None) -> None:
        ...
        self._df = self._config.verify_and_get_df(schema = self.df_schema, fallback_df = ImagenetteETL.get_dataset_df_from_hdf5())
        self._split_df = self._config.verify_and_get_split_df(df = self._df, schema = self.split_df_schema, split = self._split)
...
class ImagenetteInMemoryClassification(Dataset):
    ...
    def __init__(self, split: Literal["train", "val", "trainval", "test", "all"] = "all", config: Optional[DatasetConfig] = None) -> None:
        ...
        self._config = config or ImagenetteETL.default_config
        logger.info(f"init {self.name}[{self._split}] using\nimage_pre = {self._config.image_pre}\ntarget_pre = {self._config.target_pre}\ntrain_aug = {self._config.train_aug}\neval_aug = {self._config.eval_aug}")
        self._df = self._config.verify_and_get_df(df = self.df_schema, fallback_df = ImagenetteETL.get_dataset_df_from_hdf5())
        self._split_df = self._config.verify_and_get_split_df(df = self._df, schema = self.split_df_schema, split = self._split)
